...
折柱组合图**始终启用双 Y 轴**（柱与折线各占一条独立 Y 轴、量纲互不干扰）：
...
> 主 / 副 Y 轴的**左右位置与标签对齐**不在本图表固定，由 [坐标轴规范](../components/axes.md) 与所用主题决定（如 base 主 Y 轴在左、Ainvest 在右）。
...
- **双轴独立量纲**：柱与折线分属两条独立 Y 轴，各自独立刻度——不要强行共用一轴。
...
| | 规则 |
| --- | --- |
| ✅ | **始终启用双 Y 轴**：柱与折线各占一条独立 Y 轴（左右位置随主题 / 坐标轴规范） |
